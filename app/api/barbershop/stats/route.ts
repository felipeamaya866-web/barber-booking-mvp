...
// GET - Estadísticas completas de la barbería con filtros de periodo
...
export async function GET(req: NextRequest) {
  try {
    ...
    const [
      ...
      // Citas agrupadas por día (para gráfica)
      prisma.appointment.findMany({
        where: {
          barbershopId: bId,
          date:   { gte: rangeStart, lte: rangeEnd },
    ...
    const totalParaCancelacion = totalCitas + cancelacionesRango;
    const tasaCancelacion = totalParaCancelacion > 0
      ? Math.round((cancelacionesRango / totalParaCancelacion) * 100) : 0;
    ...
    // Agrupar citas por día para la gráfica
    const citasPorDiaMap: Record<string, number> = {};
    citasPorDia.forEach(c => {
      const key = c.date.toISOString().split('T')[0];
      citasPorDiaMap[key] = (citasPorDiaMap[key] || 0) + 1;
    });
    const graficaDias = Object.entries(citasPorDiaMap).map(([fecha, citas]) => ({
    ...
    return NextResponse.json({
      ...
      totalCitasPrev,
      variacionCitas,
      cancelaciones:   cancelacionesRango,
      tasaCancelacion,
